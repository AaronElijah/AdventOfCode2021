...
def fill_points(edge: List[List[int]]) -> List[List[int]]:
    if edge_alignment(edge) == "vertical":
        points_on_edge = [(edge[0][0], y) for y in range(min(edge[0][1], edge[1][1]), max(edge[0][1], edge[1][1])+1)]
    elif edge_alignment(edge) == "horizontal":
        points_on_edge = [(x, edge[0][1]) for x in range(min(edge[0][0], edge[1][0]), max(edge[0][0], edge[1][0])+1)]
    else:
        vector_diff = (edge[1][0]-edge[0][0], edge[1][1]-edge[0][1])
        if vector_diff[0] > 0 and vector_diff[1] > 0:
            # +ve gradient in both axis
            points_on_edge = [(edge[0][0] + i, edge[0][1] + i ) for i in range(vector_diff[0]+1)]
        elif vector_diff[0] < 0 and vector_diff[1] < 0:
            # -ve gradient in both axis
            points_on_edge = [(edge[0][0] - i, edge[0][1] - i ) for i in range(abs(vector_diff[0])+1)]
        elif vector_diff[0] > 0 and vector_diff[1] < 0:
